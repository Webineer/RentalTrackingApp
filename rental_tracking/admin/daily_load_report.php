...
//get all products for the day
    //$the_date = "2011-12-31";
    $the_date = date('Y-m-d');
    //$the_date = strtotime(date('Y-m-d', strtotime($the_date)) . " +1 day");
    $newdate = strtotime( '+1 day' , strtotime( $the_date ) ) ;
    $the_date = date( 'Y-m-j' , $newdate );
    //print $the_date . "<br>";
    //$the_date = "2011-10-11";
    print "<h2>Daily Reservation Report for " . convert_date_display($the_date) . "</h2>";
   ...
   //Get list of the reservations that meet this search criteria     
	...
	//	  display_reservation_report($res);
	   ...
	   //}
        ...
        print "<p>No products sold for today.</p>";
    }
?>
