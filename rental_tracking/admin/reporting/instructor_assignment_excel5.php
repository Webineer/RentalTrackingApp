...
<div align="center"><?php #require("../../lib/forms/instructors_assignments_list_form.php"); ?></div>
  <?php
    ...
    $sql_string4 = "select id, firstname, lastname, instructor_barcode from instructors where status = 'y' order by lastname";
    ...
    foreach ($the_date_array as $value3) {
        ...
        print "<th colspan=4>" . convert_date_display($value3) . "</th>";
        //print "<th>" . $instructor_id_hash_array[i] . "</th>";
   }
   //print "<th>Total</th></tr>";
   print "<th>L&PROG</th><th>P</th><th>PR</th><th>H</th><th>Total</th></tr>";
    ...
    foreach($instructor_id_hash_array as $key => $value1) {
        ...
        foreach($the_date_array as $value2) {
            ...
            //while ($row) {
                ...
                //$temp_write_buffer .= "<td><div align=center>" . (intval($row2[0]) + intval($row6[0])) . "&nbsp;&nbsp;|&nbsp;&nbsp;" . $row3[0] . "&nbsp;&nbsp;|&nbsp;&nbsp;" . $row5[0] . "&nbsp;&nbsp;|&nbsp;&nbsp;" . $row7[0] . "</div></td>";
                $temp_write_buffer .= "<td><div align=center>" . (intval($row2[0]) + intval($row6[0])) . "</td><td>" . $row3[0] . "</td><td>" . $row5[0] . "</td><td>" . ($row7[0]+$row8[0]+$row9[0]) . "</div></td>";
                ...
                //$employee_helper_week_total = $employee_helper_week_total + $row7[0] + $row8[0] + $row9[0];
                $employee_helper_week_total = $employee_helper_week_total + $row9[0];
            }
        ...
        $employee_grand_week_total = $employee_lesson_week_total + $employee_private_week_total + $employee_private_request_week_total + $employee_program_week_total + $employee_helper_week_total;
        ...
        //$temp_write_buffer .= "<td align=center>" . $employee_lesson_program_week_total . "(L+PROG)&nbsp;|&nbsp;" . $employee_private_week_total . "(P)&nbsp;|&nbsp;" . $employee_private_request_week_total . "(PR)&nbsp;|&nbsp;" . $employee_helper_week_total . "(H)&nbsp;|&nbsp;" . $employee_grand_week_total . "(T)</td></tr>";
        $temp_write_buffer .= "<td align=center>" . $employee_lesson_program_week_total . "</td><td>" . $employee_private_week_total . "</td><td>" . $employee_private_request_week_total . "</td><td>" . $employee_helper_week_total . "</td><td>" . $employee_grand_week_total . "</td></tr>";
    ...
    $grand_all_total = intval($grand_lesson_total) + intval($grand_private_total) + intval($grand_private_request_total) + intval($grand_program_total) + intval($grand_helper_total);
    ...
    //print "<tr><td colspan=" . (count($the_date_array)+1) . ">&nbsp;</td><th>" . $grand_lesson_program_total . "(L+PROG)|" . $grand_private_total . "(P)|" . $grand_private_request_total . "(PR)|" . $grand_helper_total . "(H)|" . $grand_all_total . "(T)</th></tr>";
    print "<tr><td colspan=" . (4*count($the_date_array)+1) . ">&nbsp;</td><th>" . $grand_lesson_program_total . "</th><th>" . $grand_private_total . "</th><th>" . $grand_private_request_total . "</th><th>" . $grand_helper_total . "</th><th>" . $grand_all_total . "</th></tr>";
    print "</table>";
