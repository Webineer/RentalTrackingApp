...
*/    
 ...
 <?php
     if ($_GET['mydate']) {
        $sql_string2 = "select product_id, location_id, instructor_id, section_name, section_time_id, max_seats, section_id, section_status, level_id, section_age_id from sections where section_date='" . $_GET['mydate'] . "'";
        print $sql_string2;
        ...
        if ($res->numRows() > 0) {
		  //enter_profile_section_data($res);
            ...
            //$field_names = array("product_id", "level_id", "location_id", "instructor_id", "section_name", "section_date", "section_time", "max_seats", "section_id", "section_status", "seats_taken");
            $field_names = array("product_id", "level_id", "location_id", "instructor_id", "section_name", "profile_id", "section_time_id", "max_seats", "section_id", "section_status", "seats_taken", "section_age_id");
            $field_values = array($row[0], $row[8], $row[1], $row[2], $row[3], '3', $row[4], $row[5], $row[6], $row[7], 0, $row[9]);
            insert_data($table_name, $field_names, $field_values);
 ...
 //list the sections for that date
    if ($_GET['mydate']) {
        ...
        //print $sql_string . "<br>";
    //} else {
        $sql_string = "select profile_sections.id, profile_sections.section_name, profile_sections.section_date, profile_sections.section_time, levels.level_name, profile_sections.section_status from profile_sections, levels, section_times where sections.level_id=levels.id and profile_sections.section_time_id=section_times.id and profile_sections.profile_id=1";
        //print $sql_string . "<br>";
