...
<%
	...
	Dim DateDelimiter, EquipmentValues, UsageValues, TotalUse
	...
	If (Request.Form("EquipmentName") <> "All") Then
		If (Request.Form("EquipmentSeries") <> "All") Then
			SQLString = "SELECT DISTINCT id, ski_number, equipment_name, id FROM equipment WHERE equipment_name='" & Request.Form("EquipmentName") & "' AND ski_number LIKE '" & Request.Form("EquipmentSeries") & "%'"
		Else
			SQLString = "SELECT DISTINCT id, ski_number, equipment_name, id FROM equipment WHERE equipment_name LIKE '" & Request.Form("EquipmentName") & "%'"
		End If
	Else
		If (Request.Form("EquipmentSeries") <> "All") Then
			SQLString = "SELECT DISTINCT id, ski_number, equipment_name, id FROM equipment WHERE ski_number LIKE '" & Request.Form("EquipmentSeries") & "%'"
		Else
			SQLString = "SELECT DISTINCT id, ski_number, equipment_name, id FROM equipment"
		End If
	End If
...
<% If IsArray(EquipmentValues) Then %>
<table border=0 cellpadding=2 cellspacing=1 bgcolor=CCCC99 width=600 align=center>
<tr><td>Internal ID</td><td>Ski Number</td><td>Name</td><td>Usage</td></tr>
<%
	...
	For Each Item In EquipmentValues
		...
		If IsArray(listedData) Then
			If (j = 4) Then
				...
				Response.Write "</tr>"
				j = 1
				TotalUse = TotalUse + CInt(listedData(0,0))
			ElseIf (j = 1) Then
		...
		End If
	Next
	Response.Write "<tr><td colspan=4>Total Use: " & CStr(TotalUse) & "</td></tr>"
	
Else
	Response.Write "<p>No Equipment in this series.</p>"
End If
%>
</table>
