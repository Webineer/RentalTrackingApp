...
<%
	...
	If Not IsArray(listedData) Then
		...
		'Call GetData(ConnectionString,SQLString)
		'If Not IsArray(listedData) Then
			'If there's not matching id, then the equipment isn't registered
...
'Define the table in the database and the database type
	...
	If IsArray(listedData) Then
		'Test to see if the equipment is rented out already
		SQLString = "SELECT max(id) FROM transactions WHERE equipment1_id='" & InternalID & "'"
		'Response.Write(SQLString) & "<br>"
		Call GetData(ConnectionString,SQLString)
		'check to see if there is a previous transaction; if so, find the transaction type
		'Response.Write(listedData(0,0) & "<br>")
		If Not IsNull(listedData(0,0)) Then
		...
		Else
			'set the transaction type to in so that you can rent the equipment
			TransType = "in"
		End If
		
		'check the transaction type to confirm that it is set to in
		If (StrComp(TransType,"out") = 0) Then
			Response.Redirect("rental_error.asp")
			'Response.Write "<p class=""warning_red"">Transaction cancelled.  This piece of equipment is already rented.  Please click <a class=""warning_red"" href=""returns.asp"">here</a> to return the equipment.</p>"
		Else
			...
			Response.Write "<p class=""warning"">OUT!</p>"
		End If
	Else
...
<script language="Javascript">setTimeout("location.assign('rentals.asp')", 2000);</script>
